...
- [ ] Consider sanitizing save names before use as localStorage keys (e.g. limit length, strip special characters)
- [ ] Consider adding structural validation when loading saves (check field types, not just existence)
...
- [ ] Fix AGENTS.md save key references - currently documents 'kroz-save' but actual keys are 'kroz-save-auto', 'kroz-save-<name>', and 'kroz-save-list' (storage.js lines 3-5). The debugging examples at AGENTS.md lines 396-400 and 467-480 that use localStorage.getItem('kroz-save') would return null
- [ ] Update AGENTS.md command registry section - currently shows a simple flat commands object but the actual system in registry.js uses aliasToAction (verb aliases to action types), commandData (command metadata with aliases and fail handlers), simpleCommands (built dynamically from commandData), complicatedCommands (complex command handlers), and knownWords (responses for recognised but unimplemented words). A developer using the documented example to add a command would not succeed
- [ ] Fix or remove COMBAT.md references - AGENTS.md references COMBAT.md in 4 locations (project structure tree line 92, object structure docs line 302, game systems section line 518, combat properties example line 535) but the file does not exist on disk. Either create it or remove the references
- [ ] Fix TESTING.md CSS selectors - uses #player-input and #game-output in the Testing Workflow and Advanced Testing Techniques sections (lines 203, 206, 213-215, 232-240, 399, 476) but actual HTML uses #input and #output. The Essential Commands section at the top (lines 31-32) correctly uses the right selectors, making the inconsistency within the same document confusing
- [ ] Update ENGINE.md file paths - all references point to helpers.js which was split into commands/helpers/environment.js, commands/helpers/mechanics.js, and commands/helpers/parsing.js. All 10 items are marked COMPLETED so this is low urgency but the stale paths could mislead anyone reading the document
- [ ] Update REQUIREDMAPDATA.md - declares all items and objects implemented but items.js still contains empty stubs (brush, extinguisher, brassHammer, redKey, blueKey, coin, hamburger, poisonedHamburger, mineralWater at lines 409-433). The lanternLit flag entry (line 195) still says "future" but the dark room system is fully implemented
- [ ] Consider archiving or removing stale tracking documents - ENGINE.md (all items completed, stale file paths), SCENERYREFORMAT.md (migration progress does not match actual state of mixed old/new format scenery), OBJECTS.md and REQUIREDMAPDATA.md (overlapping concern, outdated status). These are development-session artifacts that have served their purpose and now add cognitive load at the repository root. Could move to a docs/ directory or delete them since git preserves the history

## Code Quality
- [x] Consider splitting helpers.js into focused files (e.g. combat.js, environment.js, parsing.js) - currently 896 lines covering 30+ unrelated functions
...
- [ ] Cache buildInteractablesList() per command cycle - currently rebuilt 4-5 times per command (parsing, disambiguation, use-command resolution). Build once at the start of handleCommand and pass through, or use a module-level variable invalidated after each command
...
- [ ] Extract shared checkPassageRequirements() helper - requirement-checking logic (flag, item, roomItems) is duplicated 3 times: forward movement (movement.js:58-85), back movement (movement.js:142-175), and look() in information.js. Adding a new requirement type must be done in all 3 places. A shared function returning met/unmet status would eliminate the divergence risk
