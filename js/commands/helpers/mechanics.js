...
function processEatToKill(enemy) {
  const combat = gameState.combatState[enemy.id];
  ...
  const eatMsg = pickRandom(enemy.combat.eatMessage);
  displayText(eatMsg);

  if (combat.eatCount >= enemy.combat.requiredEats) {
    const killMsg = pickRandom(enemy.combat.killMessage);
    displayText(killMsg);
    applyEffects(enemy.combat.effects);
    delete gameState.combatState[enemy.id];
  }
      ...
      if (enemy.combat && gameState.combatState[object] && gameState.combatState[object].turnCount !== 1) {
        ...
        if (dodge) {
          ...
          if (damage.lowHealth && gameState.healthState <= damage.lowHealthThreshold) {
            ...
            healthLost =
              Math.floor(Math.random() * (damage.randomDamage[1] - damage.randomDamage[0] + 1)) +
  ...
  if (!killed && currentRoom.objects && !gameState.partCommand) {
    const hasActiveEnemy = currentRoom.objects.some((object) => {
      ...
      return (
        ...
        (enemy.combat.aggressive || gameState.combatState[object].isEngaged)
      );
    });
    if (hasActiveEnemy) {
      processEnemyTurns();
...
function processHazards(currentRoom, oldRoom) {
  if (gameState.currentRoom === oldRoom) {
    if (currentRoom.hazard) {
      if (
        (currentRoom.hazard.unless && !gameState.flags.includes(currentRoom.hazard.unless)) ||
        !currentRoom.hazard.unless
      ) {
        ...
        if (gameState.hazardState.count >= currentRoom.hazard.count) {
          ...
          if (currentRoom.hazard.damage) {
            if (gameState.healthState - currentRoom.hazard.damage < 0) {
              gameState.healthState = 0;
            } else {
